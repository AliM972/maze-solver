...
public class DrawMaze extends JFrame{
  ...
  /**
   * Creates and manages the graphical user interface for displaying a maze and its solution.
   * This class extends JFrame and is responsible for initializing the maze display, loading resources,
   * and rendering the maze, path, and other elements to the screen.
   */
  ...
  /**
   * Reads the maze layout from a file and draws the initial state of the maze.
   * @param input BufferedReader to read the maze layout.
   */
 ...
 // Draw a vertical passage
  ...
  // Draw the person figure
    ...
    /**
     * Draws an edge of the solution path between two nodes in the maze.
     * This method is used to visually represent the connection (path) between two points
     * in the maze, adjusting the drawing based on the relative positions of the nodes.
     *
     * @param u Index of the starting node of the edge in the solution path.
     * @param v Index of the ending node of the edge in the solution path.
     */
    private void drawEdge(int u, int v) {
        ...
        // Check if either of the nodes does not exist (out of bounds check)
        if (u > numNodes || v > numNodes) {
            System.out.println("Invalid edge (" + u + "," + v + ")");
            return;
        }

        // Set offset for drawing based on the room size
        offset = roomSize / 2 - 1;

        // Temporary variables for node indexes, ensuring 'u' is less than 'v'
        us = u;
        vs = v;
        if (u > v) {
            x = u; 
        ...
        // Calculate drawing coordinates based on the node positions in the maze
        x = (u % labWidth) * 2 * roomSize + offset;
        y = (u / labWidth) * 2 * roomSize + offset;

        // Attempt to erase the person icon before redrawing (for dynamic path visualization)
        if (imageFile) erasePerson((us % labWidth) * 2 * roomSize, (us / labWidth) * 2 * roomSize);

        // Determine the orientation of the edge and draw appropriately
        if (v == (u + 1)) {
            // Horizontal edge
            width = roomSize * 2;
            height = 3;
        ...
        // Set the color for the solution path and draw the edge as a filled rectangle
        display.setColor(pathColor);
        display.fillRect(x, y, width, height);

        // Redraw the person icon at the new location, based on the direction of movement
        if (imageFile) {
            if ((us % labWidth) <= (vs % labWidth))
                drawPerson((vs % labWidth) * 2 * roomSize, (vs / labWidth) * 2 * roomSize, true);
            else
                drawPerson((vs % labWidth) * 2 * roomSize, (vs / labWidth) * 2 * roomSize, false);
        }
    }
   ...
   /**
    * Draws an edge between two nodes as part of the solution visualization.
